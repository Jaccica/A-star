...
{
    return sqrt((double)(end->x-point->x)*(end->x-point->x) + (double)(end->y-point->y)*(end->y-point->y));
//    int deltaX = end->x-point->x;
//    int deltaY = end->x-point->x;
//    return kCost2*min(deltaX, deltaY) + kCost1*abs(deltaX - deltaY);
}
